...
public class YzSwiftTimer {
    ...
    public typealias SwiftTimerHandler = (YzSwiftTimer) -> Void
    ...
    public static func repeaticTimer(interval: DispatchTimeInterval, leeway: DispatchTimeInterval = .seconds(0), queue: DispatchQueue = .main , handler: @escaping SwiftTimerHandler ) -> YzSwiftTimer {
        return YzSwiftTimer(interval: interval, repeats: true, leeway: leeway, queue: queue, handler: handler)
    }
...
//
//    private let internalTimer: PTSwiftTimer
//
...
//        self.handler = handler
//        self.internalTimer = PTSwiftTimer.repeaticTimer(interval: interval, queue: queue, handler: { _ in
//        })
//        self.internalTimer.rescheduleHandler { [weak self]  PTSwiftTimer in
//            if let strongSelf = self {
